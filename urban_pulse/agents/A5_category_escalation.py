...
def category_escalation_node(state: UrbanPulseState) -> UrbanPulseState:
    ...
    # fallback action using raw data
        state["A5_output"] = {
            "items": [
                {
                    "issue_category": "General",
                    "priority": "Low",
                    "escalation_teams": ["Customer Support"],
                    "reason": "Insufficient clustering data, fallback applied",
                    "impact": {
                        "affected_reviews_percent": 100,
                        "cities": [],
                        "platforms": []
                    },
                    "supporting_reviews": []
            ...
            "meta_insight": "Fallback escalation applied due to low clustering signal"
        }
        return state
    ...
    try:
        ...
        if not actions_output:
            state["A5_output"] = {
                "items": [
                    {
                        ...
                        "priority": "Low",
                        "escalation_teams": ["Customer Support"],
                        "reason": "No strong escalation signals found",
                        "impact": {
                            "affected_reviews_percent": 0,
                            "cities": [],
                            "platforms": []
                        },
                        "supporting_reviews": []
                ...
                "meta_insight": "No critical escalation patterns detected"
            }
        else:
            state["A5_output"] = {
                "items": actions_output,
                "meta_insight": f"{len(actions_output)} escalation actions identified"
            }
        ...
        state["A5_output"] = {
        "items": [],
        "meta_insight": "Error generating business actions"
    }
