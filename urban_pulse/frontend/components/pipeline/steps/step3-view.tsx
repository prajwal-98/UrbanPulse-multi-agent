...
export default function Step3View({ data }: { data: any }) {
  ...
  const patternCount: any = out.pattern_count ?? null;
  const similarityLabel: any = out.similarity_label ?? null;

  if (!anchor && !similar.length && !theme && !extracted) return <NoData />;
    ...
    <div className="space-y-5">
      ...
      <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex gap-3">
        ...
        <p className="text-xs text-slate-600 leading-relaxed">
          ...
          then surfaces similar reviews using TF-IDF and cosine similarity —
          turning one isolated complaint into a recognisable pattern.
        </p>
      </div>
          ...
          {anchor && (
            <div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4">
                <p className="text-sm text-slate-800 leading-relaxed italic">"{anchor.text ?? "—"}"</p>
          ...
          <div>
            ...
            <div className="space-y-3">
              {similar.slice(0, 5).map((r, i) => {
                ...
                return (
                  <div key={i} className="border-l-4 border-slate-300 bg-white rounded-lg px-4 py-3">
                    ...
                    <div className="mt-2.5">
                      ...
                      <div className="flex items-center justify-between mt-1">
                        <span className="text-[10px] text-slate-500">
                          {r.city && r.platform ? `${r.platform} · ${r.city}` : (r.city || r.platform || "—")}
                        </span>
                        <span className="text-[10px] font-medium text-slate-600">{Math.round(score * 100)}%</span>
                      </div>
                    </div>
                  </div>
                );
      ...
      {theme && (
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-5 py-4">
          ...
          <p className="text-lg font-semibold text-blue-900">{typeof theme === "string" ? theme : theme.title ?? theme}</p>
        </div>
      )}
